...
<div class="page-container">
	<form action="/index.php/Admin/Category/edit?id=11" method="post" class="form form-horizontal" id="form-user-add">
			...
			<div class="formControls col-xs-6 col-sm-6">
				...
				<select name="parent_id" class="select" >
					<option value="0">顶级分类</option>
                                        <?php foreach($data as $k => $v){ if($v['id']==$info['id'] || in_array($v['id'],$children)) continue; if($v['id']== $info['parent_id']){ $select = 'selected="selected"'; }else{ $select = ''; } ?>
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">
                            ...
                            分类名称：</label>
			<div class="formControls col-xs-6 col-sm-6">
                            <input type="text" class="input-text" value="<?php echo $info['cat_name'];?>" name="cat_name" style='width:30%'/>
			</div>
		</div>
		<div class="row cl">
